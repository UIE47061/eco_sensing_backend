...
- **稽核確認流程（現行定案）**：三軌辨識完成後，後端**於確認前先算好里程（TDX／Maps）與碳排**，隨結構化欄位一併回傳，App 顯示「完整記錄確認畫面」供員工**人工稽核**。員工編輯的是**事實欄位**（票據類型／日期／起訖／金額），後端計算**衍生欄位**（里程／碳排）並以唯讀回顯；**改動起訖點或票據類型時，里程與碳排不即時重算，於員工按下確認送出前由後端以最終值重算一次**（省 API、與入庫同一時點）。員工確認後才送出、入庫，數據綁定報銷單據 ID。
- **欄位可編輯性（與 App 收據確認對話框一致）**：
  - **可編輯（事實）**：票據類型（`transport_mode`，下拉：高鐵電子票／App乘車截圖／計程車紙本收據／其他）、票據日期（`travel_date`）、起站（`origin`）、訖站（`destination`）、金額（`amount`，NT$）。
  - **衍生唯讀（後端計算）**：**碳足跡（`co2e_kg`）一律唯讀、全由後端計算**，員工不得直接編輯或覆寫（碳排＝里程×係數，兩輸入皆後端掌握；且個人碳排連動 EXP／碳幣／Shared Savings，開放手填等於開作弊後門，與 [A3]／§5.1「關鍵計算量 client 不直寫」一致）。碳足跡為 `null` 時 App 顯「尚未計算」。
  - **里程（`distance_km`）條件式可編輯**：**正常情形唯讀**，顯示 TDX／Maps 換算結果，員工不改；**唯 degraded（TDX／Maps 換算失敗、地名無法解析或外部 API 中斷）時開放員工手填**，作為人工輸入 fallback，後端以手填里程算碳排。此為「里程一律唯讀」的例外——若不開放，degraded 時整筆將因無里程而卡死、fallback 無法收尾。**公正性以配套措施處理**（非以禁止編輯處理）：手填里程之紀錄留待**後續稽核**，或於 TDX／Maps 服務回復後**由後端驗算比對**，偏差過大者標記複查。
- **語言模型中斷 fallback（現行定案）**：確認清單同時服務兩種資料狀態，UI 一套、資料流兩態——
  ...
  - **里程換算（TDX／Maps）亦 degraded**：里程欄開放手填（見上「欄位可編輯性」），碳排以手填里程計算；此為 fallback 的最末一段，確保外部里程服務中斷時稽核流程仍可收尾。
...
- **[D2] 稽核清單、LLM 中斷 fallback、里程重算時機與三態來源標記（v27 定案）**：
  ...
  - **(3) 里程/碳排「確認前顯示、送出前重算一次」**：確認前算完隨清單顯示，讓員工於稽核畫面**可見**碳足跡（唯讀，見「欄位可編輯性」——碳足跡不開放編輯）；員工改起訖／票據類型後**不即時重查**（避免每次改欄位都打 TDX／Maps），改由送出前用最終值重算一遍，與「確認後入庫」收斂於同一時點，省 API、邏輯單純。（否決即時重算：體驗略好但 API 呼叫數不可控。）
  ...
  - **(6) 碳足跡唯讀、里程條件式可編輯（欄位編輯性收斂）**：釐清「員工編輯事實、後端計算結論、結論唯讀回顯」的分工——碳排的最小必要輸入僅「交通工具別＋里程」，里程後端可由起訖自算、金額與日期不進碳排公式（日期僅供 `EMISSION_FACTOR.valid_from` 選版、金額供報銷稽核）。故：
    - **碳足跡 `co2e_kg` 一律唯讀**：為衍生值（里程×係數），非輸入值；且直連 EXP／碳幣／Shared Savings 誘因，開放手填即開作弊後門（可填 0 刷獎勵），與 [A3]／[D5]／§5.1「關鍵計算量 client 不直寫」同一原則。又 (3)「送出前後端重算」本就會覆蓋任何手填碳排，開放編輯實為假；本次一併解此隱性衝突。原 §4.1／§8.1「碳足跡可編輯」之舊述同步更正為唯讀。
    - **里程 `distance_km` 正常唯讀、degraded 時開放手填**：里程亦為衍生值，正常唯讀；但 TDX／Maps 換算失敗時，若里程亦鎖死則 degraded fallback 無法收尾（無里程即無碳排），故**唯此情形開放手填**作人工 fallback，後端以手填值算碳排。**公正性以配套措施而非禁編處理**——手填紀錄留待後續稽核、或於外部服務回復後由後端驗算比對、偏差過大者標記複查。（否決「一律唯讀」：會使 degraded 整筆卡死；否決「常態可編輯」：無 degraded 理由時開放里程等同開放碳排作弊的上游。）
    - **不分「實際／預估」雙碳值**：既然一律以後端計算為準，碳足跡收斂為**單一唯讀值**（保留「預估」語意即可），不並列「實際＋預估」兩值；App 端後續更新對齊（移除實際/預估雙欄呈現）。
...
- **掃描**：三軌單據掃描（OCR 流程，UI 完整、辨識為模擬）——**App 拍照上傳、辨識於後端**（OCR＋GPT-4o NER）；票據類型／日期／起訖／金額可編輯（事實欄位），**碳足跡唯讀、全由後端計算**（`null` 時顯「尚未計算」），里程正常唯讀、**唯 degraded 時開放手填**作 fallback（見 4.1 [D2](6)）；確認畫面即人工稽核關卡，改動起訖／票據類型者於送出前由後端以最終值重算里程與碳排，確認後走「AI 辨識中→完成獎勵」；每筆落庫記 `entry_source` 三態；LLM 中斷時清單退化為空／半填表單由員工手動補填（見 4.1 [D2]）；掃描垃圾桶（智慧回收 demo，QR→投入→AI 計算→結果動畫）；**手動上傳用紙量**（印表機路徑備選，對應 4.4 [D6]，使用者主動感測、須搭誘因；**落地定案見 4.4 [D16]**——當日可多筆記錄、每筆送出前於 App 本地編輯，App 彙總後走 `POST /api/digital-usages`（`sensing_mode='manual'`）上傳，後端一天一列）。
